# CIFRAS SIGNIFICATIVAS - ERROR RELATIVO
...
print("Xi:")
Xi = 1                     # ingresamos límite inferior(LI)
print("Xs:")
Xs = -7                     # ingresamos límite superior(LS)
print("Tol:")
Tol = 5e-5                    # ingresamos tolerancia (cifras significativas)
print("Niter:")
Niter = 100                # ingresamos número máximo de iteraciones
print("Function:")
Fun = "((3/2)*x*math.log(x+100))+((x**2)*math.log(x+100))+((9/16)*math.log(x+100))"                         # ingresamos la función a evaluar
...
if fi==0:                               # Verificamos si LI es raiz
  ...
  while E[c]>=Tol and fe!=0 and c<Niter: # mientras no lleguemos a la tolerancia o a 0 ni sobrepasemos las iteraciones
    ...
    p=(fs-fi)/(Xs-Xi)                      # Hallamos la pendiente de la recta
    Xm=(p*Xi-fi)/p                         # Encontramos el Xmedio2
    x=Xm 
    fe=eval(Fun)                           # evaluamos la f de Xm
    Error=abs((Xm-Xa)/Xm)                       # revisamos el error entre la Xanterior y la nueva Xm -> cuando la diferencia entre estas sea menor a la tolerancia coronamos
    LI.append(Xi)                          # Agregamos el valor actual de Xi
    Fi.append(fi)                          # Agregamos el valor de la funcion actual Xi
    LS.append(Xs)                          # Agregamos el valor actual de Xs
    Fs.append(fs)                          # Agregamos el valor de la funcion actual Xs
